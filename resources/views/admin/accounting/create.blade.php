  ...
  <div class="content-wrapper">
    ...
    .td{
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box box-primary">
            ...
            <div class="box-body">
              ...
              <div class="col-xs-8">
              <p class="lead">Receipt Voucher </p>

              <table style="" class="table table-striped">

                    <tr >
                    <th class="border">amount</th>
                    <th class="border">pay_no</th>
                    <th class="border">pay_date</th>
                    <th class="border">status</th>
                    </tr>

                    @foreach($ReceiptVoucher as $value )
                    <tr style="text-align: center;margin-top:5px;">
                    <td class="td">{{$value->amount}}</td>
                    <td class="td">{{$value->pay_no}}</td>
                    <td class="td">{{$value->pay_date}}</td>
                    <td class="td">{{$value->paymentType}}</td>
                    </tr>
                    @endforeach
        ...
        <div class="col-xs-12">
            @if ((($subTotal + $tax) - $datt) == 0)
                <a href="{{ route('print' , request()->route()->parameter('Accounting')) }}" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> Print</a>
            @else
        ...
        <div class="tab-content">
            <div class="tab-pane fade active in " id="home" role="tabpanel" aria-labelledby="nav-home-tab">
                ...
                <div class="text-center">
                    <h2> {{ ($subTotal + $tax) - $datt  }} </h2>
                    <span class="text-center">ريال سعودى</span>
                </div>

                <div>
                    <form action="{{ route('money')}}" method="POST" class="myform">
                    @csrf
                    @method('POST')
                        <input type="hidden" class="form-control" name="reprairCard_id"  value='{{ request()->route('Accounting') }}' placeholder="" id="reprairCard_id">
                        <input type="hidden" class="form-control" name="type"  value='cash' placeholder="" id="type">
                        <input type="hidden" class="form-control" name="tax"  value='{{ $tax }}' placeholder="" >

                        <div class="row">
                            <input type="hidden" class="form-control" name="tex_totle"  value='{{ ($subTotal + $tax) - $datt  }}' placeholder="" id="tex_totle">
                            <div class="col-md-12 " style="display: flex; justify-content: center;">
                                <div class="form-group" style="    width: 55%;">
                                    <br />
                                    <!-- <label>ادخل المبلغ المطلوب</label> -->
                                    <input  class="form-control" name="total" id="" type="number"  max="{{ ($subTotal + $tax) - $datt  }}" value="{{ old('total') }}" placeholder="ادخل المبلغ المطلوب">
                                </div>
                            </div>
            ...
            <div class="tab-pane fade  " id="profile" role="tabpanel" aria-labelledby="nav-home-tab">
                ...
                <div>
                    ...
                    @method('POST')
                        <input type="hidden" class="form-control" name="reprairCard_id"  value='{{ request()->route('Accounting') }}' placeholder="" id="reprairCard_id">
                        <input type="hidden" class="form-control" name="type"  value='cheque' placeholder="" id="type">
                        <div class="row">
                            <input type="hidden" class="form-control" name="tax"  value='{{ $tax }}' placeholder="" >
                            <input type="hidden" class="form-control" name="tex_totle"  value='{{ ($subTotal + $tax) - $datt  }}' placeholder="" id="tex_totle">
